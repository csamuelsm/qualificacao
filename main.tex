...
\begin{frame}{Influence Immunization Bounding}
	...
	\begin{minipage}[t]{0.6\textwidth}
		...
		\onslide<6->{
			We want to find a restricting set $Y \subseteq V(G)$ such that:
			\begin{itemize}
				\item<6-> $|Y| \leq l$; and
		...
		\noindent\makebox[\textwidth][c]{%
			\begin{minipage}{0.48\textwidth}
				\begin{framed}
					...
					\textit{Question}: Does $G$ have a proper $k$-coloring?\\
					\textbf{Parameter}: $k$.
				\end{framed}
			\end{minipage}\hfill\begin{minipage}{0.48\textwidth}
				\begin{framed}
					p-\textsc{Vertex-Cover}\\
					\textit{Input}: A graph $G$ and $k \in \mathbb{N}$.\\
					\textit{Question}: Does $G$ have a vertex cover of size $\leq k$?\\
					\textbf{Parameter}: $k$.
				\end{framed}
			\end{minipage}}
			...
			\begin{minipage}{0.48\textwidth}
				\begin{framed}
					p-\textsc{Clique}\\
					\textit{Input}: A graph $G$ and $k \in \mathbb{N}$.\\
					\textit{Question}: Does $G$ have a clique of size $\geq k$?\\
					\textbf{Parameter}: $k$.
				\end{framed}
			\end{minipage}\hfill\begin{minipage}{0.48\textwidth}
...
\begin{frame}{Paths with $t(v)=2$}
	...
	\begin{proposition}
		Let $G = P_n$ with $t(v)=2$ for all $v \in V(G)$. Then, for any $k \in \mathbb{N}$, \[
			\mathfrak{R}(G, t, k) \leq \mathfrak{R}_T(G, t) \leq \lceil \frac{n}{4} \rceil
		\]
	\end{proposition}
\end{frame}
...
\begin{frame}{Hardness for Bipartite Graphs}
	\begin{minipage}{.5\textwidth}
		...
		\onslide<2>{
			\begin{proposition}
				$(\mathcal{U}, \mathcal{S})$ has a set cover of size $\leq c$ if and only if $(G, t)$ has a $|\mathcal{S}|$-restricting set of size $\leq c$.
			\end{proposition}
		}
...
%%\end{frame}
%
%\begin{frame}{Pathwidth Based Bounds}
%	Let $s$ be the vertex obtained by the identification of $S$.
%	\begin{equation}
%		\label{eq:condition1}
%		k \geq |H_{T_s}(\{s\})|
%	\end{equation}
...
%			\centering
%			\begin{tikzpicture}[scale=0.8, node distance=2cm, every node/.style={circle, draw, inner sep=3pt, minimum size=4pt, scale=0.6}]
%				
%				\node[label={[rectangle, inner sep=5pt, text=red]above:2}] (A)  {A};
%				\node[label={[rectangle, inner sep=5pt, text=red]above:2}] (B) [above right of=A] {B};
%				
%				\node[fill=black, text=white] (C) [below right of=B] {C};
%				\node[fill=black, text=white] (D) [above left of=A] {D};
%				\node[fill=black, text=white] (E) [above right of=C] {E};
%				
%				\node[label={[rectangle, inner sep=5pt, text=red]above:2}] (F) [below right of=E] {F};
%				
%				\node[fill=black, text=white] (G) [below of=A] {G};
%				
%				\node[label={[rectangle, inner sep=5pt, text=red]above left:3}] (H) [below of=C] {H};
%				
%				\node[fill=black, text=white] (I) [below left of=G] {I};
%				
%				\node[label={[rectangle, inner sep=5pt, text=red]above:2}] (J) [below of=F] {J};
%				\node[label={[rectangle, inner sep=5pt, text=red]above:1}] (K) [right of=F] {K};
%				\node[label={[rectangle, inner sep=5pt, text=red]above:1}] (L) [right of=J] {L};
%				\node[label={[rectangle, inner sep=5pt, text=red]above:1}] (M) [right of=K] {M};
%				\node[label={[rectangle, inner sep=5pt, text=red]above:1}] (N) [above right of=M] {N};
%				\node[label={[rectangle, inner sep=5pt, text=red]above:1}] (O) [below right of=M] {O};
%				
%				\node[draw=none, fill=none] (P) [below right of=N] {$G$};
%				
%				\draw (A) -- (B);
%				\draw (B) -- (C);
%				\draw (A) -- (C);
...
%				\draw (H) -- (G);
%				\draw (H) -- (C);
%				\path[-] (H) edge[bend left=30] (I);
%				\path[-] (I) edge[bend left=30] (A);
%				\draw (I) -- (G);
%				\draw (H) -- (J);
%				\draw (K) -- (J);
%				\draw (K) -- (F);
...
%				\draw (M) -- (O);
%				
%				\node[rectangle, dashed, draw=green, inner sep=8pt, fit=(F)] (Q) {};
%				\node[rectangle, dashed, draw=green, inner sep=8pt, fit=(H)] (R) {};
%				\node[rectangle, dashed, draw=green, inner sep=8pt, fit=(J)] (S) {};
%				
%			\end{tikzpicture}
%			\caption{Graph $G$ with thresholds in red and the immunizing set $Y=\{F, H, J\}$ obtained from the path decomposition of $G'$ highlighted in green. We can see that it's possible to further decrease the size of this immunizing set by removing $J$, obtaining a~7-immunzing set for $G$ of size~2.}
%		\end{subfigure}\\
%		\begin{subfigure}[b]{1\textwidth}
%			\centering
%			\begin{tikzpicture}[scale=0.8, node distance=2cm, every node/.style={circle, draw, inner sep=3pt, minimum size=4pt, scale=0.6}]
%				
%				\node[label={[rectangle, inner sep=5pt, text=red]above:1}] (A)  {A};
%				\node[label={[rectangle, inner sep=5pt, text=red]above:1}] (B) [above right of=A] {B};
%				
%				\node[fill=black, text=white] (C) [below right of=B] {$s$};
%				
%				\node[label={[rectangle, inner sep=5pt, text=red]above:1}] (F) [below right of=E] {F};
%				
%				\node[label={[rectangle, inner sep=5pt, text=red]above left:1}] (H) [below of=C] {H};
%				
%				\node[label={[rectangle, inner sep=5pt, text=red]above:2}] (J) [below of=F] {J};
%				\node[label={[rectangle, inner sep=5pt, text=red]above:1}] (K) [right of=F] {K};
%				\node[label={[rectangle, inner sep=5pt, text=red]above:1}] (L) [right of=J] {L};
...
%				\draw[line width=1pt, color=red] (B) -- (C);
%				\draw[line width=1pt, color=red] (A) -- (C);
%				\draw[line width=1pt, color=red] (C) -- (F);
%				\draw[line width=1pt, color=red] (H) -- (C);
%				\draw (H) -- (J);
%				\draw (K) -- (J);
%				\draw (K) -- (F);
%				\draw (L) -- (J);
...
%				
%			\end{tikzpicture}
%			\caption{Graph $G'$ obtained with the transformation and the subgraph induced by the vertices from the bags of $T_s$ in red.}
%		\end{subfigure}\\
%		\begin{subfigure}[b]{1\textwidth}
%			\centering
%			\begin{tikzpicture}[scale=0.8, node distance=3cm, every node/.style={circle, draw, inner sep=3pt, minimum size=4pt, scale=0.6}]
%				
%				\node[rectangle, draw=blue, minimum width=50pt, inner sep=10pt, label={[rectangle, inner sep=5pt, text=blue]above:$X_i$}] (A)  {A, B, $s$};
%				
%				\node[rectangle, draw=blue, minimum width=50pt, inner sep=10pt, label={[rectangle, inner sep=5pt, text=blue]above:$X_f$}] (B) [right of=A]  {$s$, F, H};
%				
%				\node[rectangle, draw=red, minimum width=50pt, inner sep=10pt, label={[rectangle, inner sep=5pt, text=red]above:$X_{f+1}$}] (C) [right of=B]  {F, H, J};
%				
%				\node[rectangle, minimum width=50pt, inner sep=10pt] (D) [right of=C]  {F, J, L};
%				
%				\node[rectangle, minimum width=50pt, inner sep=10pt] (E) [right of=D]  {F, J, K};
%				
%				\node[rectangle, minimum width=50pt, inner sep=10pt] (F) [right of=E]  {K, M};
%				
%				\node[rectangle, minimum width=50pt, inner sep=10pt] (G) [right of=F]  {M, N, O};
%				
%				\draw (A) -- (B) -- (C) -- (D) -- (E) -- (F) -- (G);
%				
%			\end{tikzpicture}
%			\caption{The path decomposition of $G'$ with $X_i$, $X_f$, and $X_{f+1}$ highlighted.}
%		\end{subfigure}
%		\caption{A graph with thresholds, a seed set, and a immunizing set obtained from the path decomposition of $G'$.}
%		\label{fig:pw-bound-example}
%	\end{figure}
%\end{frame}
...
\begin{frame}{Conclusion and Future Work}
	...
	\graph [layered layout, level distance=1.6cm, sibling sep=1em, sibling distance=1.2cm] {
		"General Graphs"[label={[text width=2.5cm, align=left]right:{\textcolor{red}{$\W$-hard}\\}}] -> {"Complete"[label={[text width=2cm, align=center]below:{\textcolor{blue}{Polynomial $t(v)=c$}}}], "Minor-closed", "Chordal"[label={[text width=2cm, align=center]below:{\textcolor{blue}{$\W[1]$-hard}}}], "Bipartite"[label={[text width=2cm, align=center]below:{\textcolor{red}{$W[2]$-hard}\\ \textcolor{blue}{$\NP$-c}}}], "Bounded $nd(G)$"[label=below:{\textcolor{red}{$\W[1]-hard$}}], "Bounded $\Delta(G)$"[label={[text width=2.2cm, align=left]right:{\textcolor{red}{$\W[2]$-hard}}}]};
		...
		"Bounded $tw(G)$" -> "Trees"[label={[text width=2cm, align=right]left:{\textcolor{orange}{In progress...}}}];
		"Chordal" -> "Trees";
...
\begin{frame}{Conclusion and Future Work}
	\begin{problem}
	Investigate the complexity of \textsc{IIB} on other graph classes, such as cacti and $k$-regular.
	\end{problem}
	
	\begin{problem}
	Introduce and study directed and reversible versions of \textsc{IIB}.
	\end{problem}
	
	\begin{problem}
	Study graph structural parameters and modulators such as $vc(G)$, $fvs(G)$, $td(G)$ and others.
	\end{problem}
	
	\begin{problem}
	Find polynomial algorithms or show $\NP$-completeness for \textit{total restriction} with equal thresholds.
	\end{problem}
\end{frame}
